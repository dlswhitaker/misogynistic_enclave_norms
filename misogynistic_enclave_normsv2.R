...
hanovainitial <- anova(aov(h~as.factor(initialgroups),data=agents))
...
initialmean<-tapply(agents$h,agents$initialgroups,function(x) mean(x))
...
initialhvar <- tapply(agents$h,agents$initialgroups,function(x) var(x))
